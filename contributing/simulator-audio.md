...
- **Poll for new processes** (1.5s): a launched app or a freshly spawned `WebContent` is a *new* PID → push the updated set to the helper. `open`-launched helpers have no stdin, so pid updates ride the **same loopback socket** the helper streams PCM on (it's bidirectional). The helper **skips a rebuild whose pids resolve to the same audio objects** — daemon pid churn (the bulk of polls — measured ~91% no-op) doesn't change what's tapped, and rebuilding tears the tap down then recreates it, whose destroy→create gap would briefly **unmute the host** (`.muted` only applies while a tap is live). Real changes (a process starts/stops audio → `objs` change) still rebuild, so capture responsiveness is unaffected.
