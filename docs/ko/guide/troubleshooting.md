...
이 필터는 콘텐츠 필터라서 시뮬레이터 것만이 아니라 **맥의 모든 새 연결이 이 필터의 판정을 기다립니다.** 설정을 켜둔 채로 확장을 교체하면 판정하는 프로세스가 멈춥니다. 새 연결은 아무도 주지 않을 답을 기다리게 됩니다. 이미 열려 있던 연결은 그대로 동작하므로, 브라우저는 멈췄는데 다른 것은 멀쩡해 보이는 상태가 됩니다.

`tapflow migrate net-filter`는 교체본을 활성화하기 전에 필터를 먼저 끄므로 이런 일이 일어나지 않습니다. 그래도 일어난다면 필터가 꺼져 있다고 명령이 알려줍니다. 그 상태에서 네트워크는 정상이고 iOS 네트워크 제어만 빠져 있습니다.
