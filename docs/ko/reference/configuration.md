...
- 두 변수를 함께 설정합니다. 하나만 있으면 릴레이가 시작하지 않습니다.
- 비밀번호는 8자 이상이어야 합니다.
- 이미 소유자가 있는 설치에서는 아무 일도 하지 않습니다. 계정이 덮어써지지 않고 재시작할 때마다 반복되지도 않습니다.
- 설정했는데 계정 생성이 실패하면 릴레이가 시작하지 않습니다. 소유자 없는 릴레이는 루프백에 닿는 무엇이든 차지할 수 있어서 그대로 서비스하는 것보다 멈추는 편이 안전합니다.
