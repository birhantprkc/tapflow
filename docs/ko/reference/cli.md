...
**필터가 실제로 돌기 시작했는지 확인한 뒤에 성공을 말합니다.** 확장을 설치할 때 macOS가 답하는 것은 "거절하지 않았다"이지 "동작한다"가 아닙니다. 설정은 그 뒤에 필터에게 전달되고 아무것도 돌아오지 않습니다. 그래서 최대 30초까지 지켜보다가 필터가 나타나면 바로 끝냅니다. 나타나지 않으면 성공이라고 하지 않고 그렇게 말합니다. 어느 쪽이든 맥의 네트워크는 영향을 받지 않습니다. 돌지 않는 필터는 아무것도 막지 않기 때문입니다.
